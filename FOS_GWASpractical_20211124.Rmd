...
### 3.1 PHENOTYPE DATA
```{r phenotypes}
pheno <- read.table ("/cloud/project/data/Mydata.pheno", header=TRUE, stringsAsFactors=F)
dim (pheno)
head (pheno)

covar <- read.table ("/cloud/project/data/Mydata.covar", header=TRUE, stringsAsFactors=F)
dim (covar)
head (covar)
```
...
vcf_header <- readLines("/cloud/project/data/Mydata.vcf")
head(vcf_header)
  
# Read the file
vcf <- fread ("/cloud/project/data/Mydata.vcf")
head (vcf)
...
```{r load results}
results <- read.table (gzfile("/cloud/project/results/Myresults.txt.gz"),header=TRUE, stringsAsFactors=F)
